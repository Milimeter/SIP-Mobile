...
class _MyRegisterWidget extends State<RegisterWidget>
  ...
  void _loadSettings() async {
    this.setState(() {
      _wsUriController.text = box.read('ws_uri') ?? 'wss://tryit.jssip.net:10443';
      _sipUriController.text = box.read('sip_uri') ?? 'hello_flutter@tryit.jssip.net';
      _displayNameController.text = box.read('display_name') ?? 'Flutter SIP UA';
      
      _passwordController.text = box.read('password');
      _authorizationUserController.text = box.read('auth_user');
    });
  }

  void _saveSettings() {
    box.write('ws_uri', _wsUriController.text);
    box.write('sip_uri', _sipUriController.text);
    box.write('display_name', _displayNameController.text);
    box.write('password', _passwordController.text);
    box.write('auth_user', _authorizationUserController.text);
  }
